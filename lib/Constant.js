...
function initAddressInfo(SDK_ENV) {
    ...
    if ('prod' === SDK_ENV || 'prod_dev' === SDK_ENV) {
        ...
        addressInfo.bananaGraphApi = 'https://api.apex.exchange/g1/subgraphs/name/apex/banana-staking';
...
async function initNetAddress(SDK_ENV, apexBaseApi = '') {
    ...
    if ('prod' === SDK_ENV || 'prod_dev' === SDK_ENV) {
        ...
        addressInfo.configApi = baseApi;
        addressInfo.airdropApi = baseApi;
        addressInfo.bananaGraphApi = 'https://api.apex.exchange/g1/subgraphs/name/apex/banana-staking';
        addressInfo.chainId = 1;
        addressInfo.scan = 'https://etherscan.io';
        addressInfo.rpc = 'https://eth-mainnet.g.alchemy.com/v2/KF-p7VWklxoDsAcNnWg9GFEWs1KNfYtB';
        ...
        if (typeof window !== 'undefined') {
            addressInfo.configApi = lodash_1.default.get(window, 'location.origin', baseApi);
            addressInfo.airdropApi = lodash_1.default.get(window, 'location.origin', baseApi);
        }
        else {
            addressInfo.configApi = baseApi;
            addressInfo.airdropApi = baseApi;
    ...
    addressInfo.stakingPool = res.data.contractInfoList.filter((it) => it.key === 'staking_pool').map((it) => it.address)[0] || '';
    addressInfo.bananaGraphApi = res.data.contractInfoList.filter((it) => it.key === 'graph_url').map((it) => it.address)[0] || '';
    addressInfo.rpc = res.data.rpc.url;
    // addressInfo.getApiService();
    // addressInfo.readonlyConnectInfo();
    currentAddressInfo = addressInfo;
    service_1.Trace.debug('INIT NODE_ENV', SDK_ENV);
