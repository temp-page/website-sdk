...
async function initNetAddress(SDK_ENV) {
    ...
    if ('prod' === SDK_ENV || 'prod_dev' === SDK_ENV) {
        ...
        // 默认环境
        addressInfo.configApi = 'https://dev.pro.apex.exchange';
        addressInfo.airdropApi = 'https://dev.pro.apex.exchange';
        addressInfo.bananaGraphApi = 'https://g-api.apex.exchange/subgraphs/name/apex/banana-dev';
        addressInfo.chainId = 5;
        addressInfo.scan = 'https://goerli.etherscan.io';
        addressInfo.rpc = 'https://goerli.infura.io/v3/9aa3d95b3bc440fa88ea12eaa4456161';
    ...
    const res = await baseApi.request(addressInfo.configApi + '/api/v1/trade-mine/contracts', 'get', {});
    addressInfo.banana = res.data.metaData.banana.address;
    addressInfo.bananaClaimable = res.data.metaData.banana_claimable.address;
    addressInfo.apex = res.data.metaData.apex.address;
    addressInfo.usdc = res.data.metaData.usdc.address;
    addressInfo.pair = res.data.metaData.twamm_pair.address;
    addressInfo.twammRouter = res.data.metaData.twamm.address;
    addressInfo.buybackPool = res.data.metaData.buyback_pool.address;
    addressInfo.exApex = res.data.metaData.es_apex.address;
    // addressInfo.getApiService();
    // addressInfo.readonlyConnectInfo();
    currentAddressInfo = addressInfo;
    service_1.Trace.debug('INIT NODE_ENV', SDK_ENV);
