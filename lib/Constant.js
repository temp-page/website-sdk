...
async function initNetAddress(SDK_ENV, apexBaseApi = '') {
    ...
    addressInfo.stakingPool =
        res.data.contractInfoList.filter((it) => it.key === 'staking_pool').map((it) => it.address)[0] || '';
    addressInfo.apexPool =
        res.data.contractInfoList.filter((it) => it.key === 'apex_pool').map((it) => it.address)[0] || '';
    addressInfo.bananaGraphApi =
        res.data.contractInfoList.filter((it) => it.key === 'graph_url').map((it) => it.address)[0] ||
            addressInfo.bananaGraphApi;
