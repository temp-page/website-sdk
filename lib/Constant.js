...
async function initNetAddress(SDK_ENV) {
    ...
    if ('prod' === SDK_ENV || 'prod_dev' === SDK_ENV) {
        ...
        // 默认环境
        addressInfo.configApi = lodash_1.default.get(window.location.origin, 'https://dev.pro.apex.exchange');
        addressInfo.airdropApi = lodash_1.default.get(window.location.origin, 'https://dev.pro.apex.exchange');
        addressInfo.bananaGraphApi = 'https://g-api.apex.exchange/subgraphs/name/apex/banana-dev';
        addressInfo.chainId = 5;
        addressInfo.scan = 'https://goerli.etherscan.io';
        addressInfo.rpc = 'https://goerli.infura.io/v3/9aa3d95b3bc440fa88ea12eaa4456161';
    ...
    const baseApi = new service_1.BaseApi();
    const res = await baseApi.request(addressInfo.configApi + '/api/v1/mine/contracts', 'get', {});
    addressInfo.banana = res.data.contractInfoList.filter(it => it.key === "banana").map(it => it.address)[0] || "";
    addressInfo.bananaClaimable = res.data.contractInfoList.filter(it => it.key === "banana_claimable").map(it => it.address)[0] || "";
    ...
    addressInfo.twammRouter = res.data.contractInfoList.filter(it => it.key === "twamm").map(it => it.address)[0] || "";
    addressInfo.buybackPool = res.data.contractInfoList.filter(it => it.key === "buyback_pool").map(it => it.address)[0] || "";
    addressInfo.exApex = res.data.contractInfoList.filter(it => it.key === "es_apex").map(it => it.address)[0] || "";
    // addressInfo.getApiService();
    // addressInfo.readonlyConnectInfo();
    currentAddressInfo = addressInfo;
    service_1.Trace.debug('INIT NODE_ENV', SDK_ENV);
