...
let RequestService = class RequestService {
    ...
    async apexPoolStakeInfo(address, privateApi) {
        const round = parseInt(lodash_1.default.get(await privateApi.get(`/api/v1/staking/current-staking-round`, {}), 'data.round', '0'), 10);
        ...
        if (finishedStakingRoundList.length > 0) {
            ...
            const accountRewardByIdResp = await privateApi.get(`/api/v1/account-reward-by-id?rewardIds=${rewardIds}`, {});
            const accountRewardList = Array.from(lodash_1.default.get(accountRewardByIdResp, 'data.rewardList', []));
