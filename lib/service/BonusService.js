...
let BonusService = class BonusService extends BaseService_1.BaseService {
    ...
    async esApexInfo() {
        ...
        bonusEsApex.withdrawnVestInfos = bonusEsApex.vestInfos.filter((it) => !it.forceWithdrawn);
    ...
    getBonusEsApexVestInfo(vestInfos, exApexInfo, blockTime) {
        return vestInfos.map((it, index) => {
            ...
            if (vestInfo.forceWithdrawn || new bignumber_js_1.default(vestInfo.unlocked).comparedTo(vestInfo.esApexAmount) === 0) {
