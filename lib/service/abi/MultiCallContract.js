...
let MultiCallContract = class MultiCallContract extends BaseService_1.BaseService {
    constructor(connectInfo) {
        super(connectInfo);
        const address = connectInfo.isArbi()
            ? this.connectInfo.addressInfo.arbiMulticall
            : connectInfo.isMantle()
                ? this.connectInfo.addressInfo.mantleMulticall
                : this.connectInfo.addressInfo.multicall;
        this.multicall2Instance = new mulcall_1.MulContract(address, abi_1.Multicall2);
        this.multiCallInstance = new mulcall_1.Provider(this.connectInfo.provider, address);
    }
    ...
    async call(...shapeWithLabels) {
        ...
        const res = calls.length > 0 ? await this.multiCallInstance.all(calls) : [];
        let index = 0;
