...
let MerkleDistributor = class MerkleDistributor extends BaseService_1.BaseService {
    ...
    async claim(index, account, amount, merkleProof) {
        ...
        return this.connectInfo.tx().sendContractTransaction(this.merkleDistributorContract, 'claim', args, {});
    }
    ...
    async init(user, token, data) {
        ...
        const self = data.find((it) => it[1].toLowerCase() === user.toLowerCase());
        if (!self) {
        ...
        merkleTreeResult.showAmount = new bignumber_js_1.BigNumber(self[2])
            .div(10 ** tokenInfo.decimal)
            .toFixed(tokenInfo.decimal, bignumber_js_1.BigNumber.ROUND_DOWN);
        merkleTreeResult.isClaim = await this.isClaimed(Number(self[0]).toString());
        ...
        if (!merkleTreeResult.isClaim) {
            merkleTreeResult.claim = async () => {
                ...
                const proof = balanceTree.getProof(Number(self[0]).toString(), self[1], self[2]);
                tool_1.Trace.debug('PARAMS', Number(self[0]).toString(), self[1], self[2], proof);
                return this.claim(Number(self[0]).toString(), self[1], self[2], proof);
            };
        }
        return merkleTreeResult;
    }
     ...
     * @param proof
     */
    async initApiData(user, token, isExist, index, amount, proof) {
        const [tokenInfo] = await this.connectInfo.erc20().getErc20Info(token);
        const merkleTreeResult = new vo_1.MerkleTreeResult();
        if (!isExist) {
            merkleTreeResult.showAmount = amount;
            merkleTreeResult.isClaim = false;
            merkleTreeResult.isExist = false;
            return merkleTreeResult;
        }
        merkleTreeResult.showAmount = amount;
        merkleTreeResult.isClaim = await this.isClaimed(Number(index).toString());
        merkleTreeResult.isExist = true;
        if (!merkleTreeResult.isClaim) {
            merkleTreeResult.claim = async () => {
                const value = new bignumber_js_1.BigNumber(amount).multipliedBy(10 ** tokenInfo.decimal).toFixed(0, bignumber_js_1.BigNumber.ROUND_DOWN);
                return this.claim(Number(index).toString(), user, value, proof);
            };
        }
        return merkleTreeResult;
    }
...
__decorate([
    (0, tool_1.EnableLogs)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, Boolean, Number, String, Array]),
    __metadata("design:returntype", Promise)
], MerkleDistributor.prototype, "initApiData", null);
...
class BalanceTree {
    constructor(balances) {
        this.tree = new merkletreejs_1.MerkleTree(balances.map(([index, account, amount]) => {
