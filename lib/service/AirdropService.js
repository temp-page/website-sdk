...
let AirdropService = class AirdropService extends BaseService_1.BaseService {
    ...
    async backtrackingAirdrop(user = this.connectInfo.account) {
        ...
        // }
        const [airDropInfo, { airdrop_contract, claim_end_time, claim_start_time, stat_end_time }, apexPriceResult] = await Promise.all([
            this.connectInfo
                .api()
        ...
        merkleTreeResult.totalUsd = new bignumber_js_1.default(merkleTreeResult.usdPrice).multipliedBy(merkleTreeResult.showAmount).toFixed();
        merkleTreeResult.isJoined = new bignumber_js_1.default(airDropInfo.volume).comparedTo("0") > 0 || new bignumber_js_1.default(airDropInfo.deposit).comparedTo("0") > 0;
