...
let AirdropService = class AirdropService extends BaseService_1.BaseService {
    ...
    async backtrackingAirdrop(user = this.connectInfo.account) {
        ...
        const [airDropInfo, { airdrop_contract }] = await Promise.all([
            this.connectInfo.api().baseApi.apiBaseRequest(this.connectInfo.addressInfo.airdropApi + "/api/airdrop/getPlayer", "post", {
                "my_wallet": user
            }),
            this.connectInfo.api().baseApi.apiBaseRequest(this.connectInfo.addressInfo.airdropApi + "/api/airdrop/get-airdrop-config", "post", {})
        ]);
