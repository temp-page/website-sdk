...
let DragonApi = class DragonApi {
    ...
    // 榜单
    async top() {
        const dragonTopUsers = await this.baseApi.arbiDragonNftGraph((0, gql_1.getDragonTopUsers)(), {});
        // API
        const lotteryRewardList = await this.baseApi.apiBaseRequest(`/api/v2/reward/lottery-reward-trade`, 'get', {
            rewardId: '100100',
            token: 'usdc',
            ethAddress: dragonTopUsers.userInfos.map(it => it.id.toLowerCase()).join(",")
        });
        const connectInfo = this.baseApi.arbiConnectInfo();
        const divineDragoNft = connectInfo.create(abi_1.IDivineDragoNft);
        const rewardTradeList = lodash_1.default.get(lotteryRewardList, 'rewardTradeList') || [];
        const groupByRewardTradeList = lodash_1.default.groupBy(rewardTradeList.map(it => {
            return {
                ...it,
                ethAddress: it.ethAddress.toLowerCase()
            };
        }), 'ethAddress');
        const list = dragonTopUsers.userInfos.map((item, index) => {
            return {
                ethAddress: item.id.toLowerCase(),
                tradeAmount: groupByRewardTradeList[item.id.toLowerCase()] ? new bignumber_js_1.default(groupByRewardTradeList[item.id.toLowerCase()][0].tradeAmount).toFixed() : '0',
                index: index + 1,
                divineDragonCount: new bignumber_js_1.default(item.dragon).toFixed(),
                mintCount: new bignumber_js_1.default(item.totalStar).toFixed()
            };
        });
        ...
        return {
            ...
            divineDragoCount,
            mintDragoBallCount: lodash_1.default.get(lotteryRewardList, 'mintCount', '0'),
            list
        };
    }
    async myNft(privateApi, connectInfo) {
        const dragonBallNfts = connectInfo.addressInfo.dragonBall.DragonBallNfts;
        const result = {
            available: dragonBallNfts.map((it, index) => {
                return {
                    url: connectInfo.addressInfo.chainId === 1 ? `https://opensea.io/assets/arbitrum/${it}` : `https://testnets.opensea.io/assets/arbitrum-sepolia/${it}`,
        ...
        result.available = nftTokenIds.map((it, index) => {
            ...
            return {
                url: result.available[index].url,
                star: new bignumber_js_1.default(index + 1).toFixed(),
                count: list.length,
                mintData: list
            };
        ...
        result.mintAllDragonBall = async () => {
            ...
            // 取前100个
            return ddDragonBall.batchMint(Array.from(data).slice(0, 100).map(it => it.star), Array.from(data).slice(0, 100).map(it => it.tokenId), Array.from(data).slice(0, 100).map(it => it.sign));
        };
