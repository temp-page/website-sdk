...
let DragonApi = class DragonApi {
    ...
    async top() {
        ...
        const list = (lodash_1.default.get(lotteryRewardList, 'rewardTradeList') || []).map((item, index) => {
            return {
                ...item,
                index: index + 1,
                divineDragonCount: new bignumber_js_1.default(dragonTopUsers.userInfos[index].dragon).toFixed(),
                mintCount: new bignumber_js_1.default(dragonTopUsers.userInfos[index].totalStar).toFixed()
            };
        });
