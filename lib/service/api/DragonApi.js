...
let DragonApi = class DragonApi {
    ...
    async top() {
        // API
        const lotteryRewardList = await this.baseApi.apiBaseRequest(`/api/v2/reward/lottery-reward-list?rewardId=100060&channel=1`, 'get', {});
    ...
    async myNft(privateApi, connectInfo) {
        const result = {
            available: Array.from({ length: 7 }).map((_it, index) => {
                return {
                    star: new bignumber_js_1.default(index).toFixed(),
                    count: 0,
                    mintData: []
                };
            }),
        ...
        const lotteryRewardRecord = await privateApi.get('/v2/reward/lottery-reward-record', { rewardId: '100060' });
        const nftList = lodash_1.default.get(lotteryRewardRecord, 'list') || [];
        const divineDragoNft = connectInfo.create(abi_1.IDivineDragoNft);
        const dDragonBall = connectInfo.create(abi_1.IDragonBall);
        ...
        nftList.forEach((item, index) => {
            if (checkMintList[index].checkMint) {
                mintList.push(item);
            }
            else {
        ...
        const mintStarGroup = lodash_1.default.groupBy(mintList, item => item.star);
        const noMintStarGroup = lodash_1.default.groupBy(noMintList, item => item.star);
        result.dragonCount = parseInt(balanceOf, 10);
        result.available = Object.entries(mintStarGroup).map(([key, value]) => {
            const list = value;
            return {
                star: key,
                count: list.length,
