...
let DragonApi = class DragonApi {
    ...
    async myNft(privateApi, connectInfo) {
        ...
        result.mintAllDragonBall = async () => {
            ...
            // 取前100个
            return ddDragonBall.batchMint(Array.from(data).splice(100).map(it => it.star), Array.from(data).splice(100).map(it => it.tokenId), Array.from(data).splice(100).map(it => it.sign));
        };
