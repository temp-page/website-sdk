...
let DragonApi = class DragonApi {
    ...
    async top() {
        ...
        const lotteryRewardList = await this.baseApi.apiBaseRequest(`/api/v2/reward/lottery-reward-trade`, 'get', {
            rewardId: '100100',
            token: 'usdc',
            ethAddress: dragonTopUsers.userInfos.map(it => it.id).join(",")
        });
        ...
        const groupByRewardTradeList = lodash_1.default.groupBy(rewardTradeList, 'ethAddress');
        const list = dragonTopUsers.userInfos.map((item, index) => {
            return {
                ethAddress: item.id,
                tradeAmount: groupByRewardTradeList[item.id] ? new bignumber_js_1.default(groupByRewardTradeList[item.id][0].tradeAmount).toFixed() : '0',
                index: index + 1,
    ...
    async myNft(privateApi, connectInfo) {
        ...
        const result = {
            available: dragonBallNfts.map((it, index) => {
                return {
                    url: connectInfo.addressInfo.chainId === 1 ? `https://testnets.opensea.io/assets/sepolia/${it}` : `https://testnets.opensea.io/assets/arbitrum-sepolia/${it}`,
