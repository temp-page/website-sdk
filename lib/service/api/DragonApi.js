...
let DragonApi = class DragonApi {
    ...
    async myNft(privateApi, connectInfo) {
        ...
        const lotteryRewardRecord = await privateApi.get('/api/v2/reward/lottery-reward-record', { rewardId: '100100', token: 'usdc', ethAddress: connectInfo.account });
        const nftList = lodash_1.default.get(lotteryRewardRecord, 'data.rewardList') || [];
        const divineDragoNft = connectInfo.create(abi_1.IDivineDragoNft);
        const dDragonBall = connectInfo.create(abi_1.IDragonBall);
