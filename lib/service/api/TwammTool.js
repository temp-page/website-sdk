...
function computeVirtualBalances(tokenAStart, tokenBStart, tokenAIn, tokenBIn) {
    ...
    //   tokenBIn.isZero()
    // ) {
    tokenAOut = tokenAStart.add(tokenAIn).mul(tokenBIn).div(tokenBStart.add(tokenBIn));
    tokenBOut = tokenBStart.add(tokenBIn).mul(tokenAIn).div(tokenAStart.add(tokenAIn));
...
async function executeVirtualOrders(connectInfo) {
    ...
    const expiriesList = pairResult.getExpiriesSinceLastExecuted || [];
    ...
    for (let i = 0; i < expiriesList.length; i++) {
        ...
        if ((iOrderPoolASalesRateEnding.gt(bigZero) || iOrderPoolBSalesRateEnding.gt(bigZero)) &&
            ...
            // tslint:disable-next-line:no-shadowed-variable
            const tokenASellAmount = currentSalesRateA.mul(blockNumberIncrement).div(10000);
            // tslint:disable-next-line:no-shadowed-variable
            const tokenBSellAmount = currentSalesRateB.mul(blockNumberIncrement).div(10000);
            // tslint:disable-next-line:no-shadowed-variable
            ...
            if (!currentSalesRateA.eq(0)) {
                rewardFactorA = rewardFactorA.add(ethers_1.utils.parseUnits(tokenBOut.toString(), 18).div(currentSalesRateA).mul(10000));
            }
            if (!currentSalesRateB.eq(0)) {
                rewardFactorB = rewardFactorB.add(ethers_1.utils.parseUnits(tokenAOut.toString(), 18).div(currentSalesRateB).mul(10000));
            }
            // update state
