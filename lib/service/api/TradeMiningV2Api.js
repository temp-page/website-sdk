...
let TradeMiningV2Api = class TradeMiningV2Api {
    ...
    async getActionsHistory(user, actions = actionTypes) {
        const { userActionTotalGains, bananaUserActionsLogs } = await this.baseApi.bananaV2Graph((0, gql_1.getActionsV2History)(), {
            user,
            actions
        });
        ...
        tradeMiningTotalGains.totalSwaped = lodash_1.default.get(userActionTotalGains, "[0].usdt", "0");
    ...
    async burnRecord() {
        const { transferLogs } = await this.baseApi.bananaV2Graph((0, gql_1.getBananaBurnRecord)(), {});
