...
let Web3WalletApi = class Web3WalletApi {
    constructor(baseUrl, token, riskToken = "") {
    ...
    async request(path, method, data, config = {
        ...
        config.headers.riskToken = this.riskToken;
        const result = await this.baseApi.request(this.baseUrl + path, method, data, config);
        if (result.ret_code > 0) {
            if (result.ret_code === 32000) {
                throw new BasicException_1.BasicException(result.ret_msg, null, BasicException_1.BasicException.SAFE_CHECK, result.result);
            }
            throw new BasicException_1.BasicException(result.ret_msg, null, result.ret_code);
        }
