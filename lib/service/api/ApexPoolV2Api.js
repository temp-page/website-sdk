...
let ApexPoolV2Api = class ApexPoolV2Api {
    ...
    async apexPoolAPY() {
        ...
        const avgRegularTime = (0, tool_1.getValue)(apiStakeInfo, 'avgRegularTime', '0');
        return {
            apy,
            totalApexStake,
            totalEsApexStake,
    ...
    async apexPoolStakeInfo(address, privateApi) {
        ...
        const avgRegularTime = (0, tool_1.getValue)(stakingUserData, 'avgRegularTime', '0');
    ...
    async stakeInfo(accountId, privateApi, connectInfo) {
        ...
        const apexPoolStakeInfo = {
            ...
            stakingHoldingsList: apiStakeInfo.stakingHoldingsList,
            ...
            userRegularCanUnStakeApex: userStake.stakeInfos.filter(it => new bignumber_js_1.default(it.stakeTime).plus(it.lockPeriod).comparedTo(new Date().getTime()) < 0 && it.tokenName === 'apex').map(it => it.amount).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0'),
            userRegularCanUnStakeEsApex: userStake.stakeInfos.filter(it => new bignumber_js_1.default(it.stakeTime).plus(it.lockPeriod).comparedTo(new Date().getTime()) < 0 && it.tokenName === 'esApex').map(it => it.amount).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0'),
            ...
            unStakeRegular: (token) => {
                const stakeIds = userStake.stakeInfos.filter(it => new bignumber_js_1.default(it.stakeTime).plus(it.lockPeriod).comparedTo(new Date().getTime()) < 0 && it.tokenName === 'apex').map(it => it.stakeId);
