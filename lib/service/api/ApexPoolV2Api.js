...
let ApexPoolV2Api = class ApexPoolV2Api {
    ...
    async apexPoolStakeInfo(address, privateApi) {
        const [stakingUserDataResp] = await Promise.all([
        ...
        // 总的 活期+定期
        const userTotalApexStake = stakingHoldingsList
            .filter(it => it.stakeToken === 'apex' && new bignumber_js_1.default(it.totalFactor).comparedTo("0") > 0)
            .map(it => it.stakeAmount).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0');
        const userTotalEsApexStake = stakingHoldingsList
            .filter(it => it.stakeToken === 'esApex' && new bignumber_js_1.default(it.totalFactor).comparedTo("0") > 0)
            .map(it => it.stakeAmount).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0');
        // 活期
        const userCurrentTotalApexStake = stakingHoldingsList
            .filter(it => it.stakeToken === 'apex' && new bignumber_js_1.default(it.totalFactor).comparedTo("0") > 0 && it.isLockupStaking === false)
            .map(it => it.stakeAmount).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0');
        const userCurrentTotalEsApexStake = stakingHoldingsList
            .filter(it => it.stakeToken === 'esApex' && new bignumber_js_1.default(it.totalFactor).comparedTo("0") > 0 && it.isLockupStaking === false)
            .map(it => it.stakeAmount).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0');
        // 定期
        const userRegularTotalApexStake = stakingHoldingsList
            .filter(it => it.stakeToken === 'apex' && new bignumber_js_1.default(it.totalFactor).comparedTo("0") > 0 && it.isLockupStaking)
            .map(it => it.stakeAmount).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0');
        const userRegularTotalEsApexStake = stakingHoldingsList
            .filter(it => it.stakeToken === 'esApex' && new bignumber_js_1.default(it.totalFactor).comparedTo("0") > 0 && it.isLockupStaking)
            .map(it => it.stakeAmount).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0');
    ...
    async stakeInfo(accountId, privateApi, connectInfo) {
        ...
        const apexPoolStakeInfo = {
            ...
            unStakeRegular: (token) => {
                const stakeIds = userStake.stakeInfos.filter(it => new bignumber_js_1.default(it.stakeTime).plus(it.lockPeriod).comparedTo(new Date().getTime() / 1000) < 0 && it.tokenName === token).map(it => it.stakeId);
