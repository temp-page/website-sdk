...
let ApexPoolV2Api = class ApexPoolV2Api {
    ...
    async apexPoolAPY() {
        ...
        const [apexPrice, apiStakeInfo, mainnetApexPoolStakeInfos, arbiApexPoolStakeInfos, mainnetRegularApexPoolStakeInfos, arbiRegularApexPoolStakeInfos, mantleRegularApexPoolStakeInfos] = await Promise.all([
            ...
            this.baseApi.graphBase(apexRegularStakingConfig.mantle.graphUrlV3, (0, gql_1.getApexPoolStakeInfos)(), {}),
        ]);
        ...
        const mantleRegularTotalApexStake = (0, tool_1.getValue)(mantleRegularApexPoolStakeInfos.apexPoolStakeInfos, '[0].apex', '0');
        const mantleRegularTotalEsApexStake = (0, tool_1.getValue)(mantleRegularApexPoolStakeInfos.apexPoolStakeInfos, '[0].esApex', '0');
        const totalApexStake = new bignumber_js_1.default(mainnetTotalApexStake).plus(arbiTotalApexStake).plus(mainnetRegularTotalApexStake).plus(arbiRegularTotalApexStake).plus(mantleRegularTotalApexStake).toFixed();
        const totalEsApexStake = new bignumber_js_1.default(mainnetTotalEsApexStake).plus(arbiTotalEsApexStake).plus(mainnetRegularTotalEsApexStake).plus(arbiRegularTotalEsApexStake).plus(mantleRegularTotalEsApexStake).toFixed();
        ...
        const totalLockupAvgStakeTime = new bignumber_js_1.default((0, tool_1.getValue)(apiStakeInfo, 'apexStakingApyFactor.totalLockupAvgStakeTime', '0')).toFixed();
        return {
            apy,
            totalApexStake,
            totalEsApexStake,
            ...
            avgRegularTime,
            totalLockupAvgStakeTime
        };
    }
    async apexPoolStakeInfo(address, privateApi) {
        const apexRegularStakingConfig = this.baseApi.address().apexRegularStaking;
        const [mainnetApexPoolStakeInfos, arbiApexPoolStakeInfos, mainnetApexPoolRegularStakeInfos, arbiApexPoolRegularStakeInfos, mantleApexPoolRegularStakeInfos, stakingUserDataResp] = await Promise.all([
            ...
            this.baseApi.graphBase(apexRegularStakingConfig.common.graphUrlV3, (0, gql_1.getUserApexPoolStakeInfosV2)(), { user: address }),
            this.baseApi.graphBase(apexRegularStakingConfig.arbi.graphUrlV3, (0, gql_1.getUserApexPoolStakeInfosV2)(), { user: address }),
            this.baseApi.graphBase(apexRegularStakingConfig.mantle.graphUrlV3, (0, gql_1.getUserApexPoolStakeInfosV2)(), { user: address }),
        ...
        const mantleRegularTotalApexStake = (0, tool_1.getValue)(mantleApexPoolRegularStakeInfos.apexPoolStakeInfos, '[0].apex', '0');
        const mantleRegularTotalEsApexStake = (0, tool_1.getValue)(mantleApexPoolRegularStakeInfos.apexPoolStakeInfos, '[0].esApex', '0');
        ...
        const userMantleRegularTotalApexStake = (0, tool_1.getValue)(mantleApexPoolRegularStakeInfos.userStakeInfos, '[0].apex', '0');
        const userMantleRegularTotalEsApexStake = (0, tool_1.getValue)(mantleApexPoolRegularStakeInfos.userStakeInfos, '[0].esApex', '0');
        const totalApexStake = new bignumber_js_1.default(mainnetTotalApexStake).plus(arbiTotalApexStake).plus(mainnetRegularTotalApexStake).plus(arbiRegularTotalApexStake).plus(mantleRegularTotalApexStake).toFixed();
        const totalEsApexStake = new bignumber_js_1.default(mainnetTotalEsApexStake).plus(arbiTotalEsApexStake).plus(mainnetRegularTotalEsApexStake).plus(arbiRegularTotalEsApexStake).plus(mantleRegularTotalEsApexStake).toFixed();
        const userTotalApexStake = new bignumber_js_1.default(userMainnetTotalApexStake).plus(userArbiTotalApexStake).plus(userMainnetRegularTotalApexStake).plus(userArbiRegularTotalApexStake).plus(userMantleRegularTotalApexStake).toFixed();
        const userTotalEsApexStake = new bignumber_js_1.default(userMainnetTotalEsApexStake).plus(userArbiTotalEsApexStake).plus(userMainnetRegularTotalEsApexStake).plus(userArbiRegularTotalEsApexStake).plus(userMantleRegularTotalEsApexStake).toFixed();
        ...
        const apexStake = {
            ...
            totalLockupAvgStakeTime: (0, tool_1.getValue)(stakingUserData, 'apexStakingApyFactor.totalLockupAvgStakeTime', '0'),
        };
        ...
        const totalLockupAvgStakeTime = apexStake.totalLockupAvgStakeTime;
        // 用户定期可以移除的apex
        const userRegularCanUnStakeApex = [mainnetApexPoolRegularStakeInfos, arbiApexPoolRegularStakeInfos, mantleApexPoolRegularStakeInfos]
            .flatMap(it => it.stakeInfos)
            .filter(it => new bignumber_js_1.default(it.stakeTime).plus(it.lockPeriod).comparedTo(new Date().getTime() / 1000) < 0)
            .map(it => it.amount)
            .reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0');
        const userRegularTotalApexStake = new bignumber_js_1.default(userMainnetRegularTotalApexStake).plus(userArbiRegularTotalApexStake).plus(userMantleRegularTotalApexStake).minus(userRegularCanUnStakeApex).toFixed();
        const userRegularTotalEsApexStake = new bignumber_js_1.default(userMainnetRegularTotalEsApexStake).plus(userArbiRegularTotalEsApexStake).plus(userMantleRegularTotalEsApexStake).toFixed();
    ...
    async stakeInfo(accountId, privateApi, connectInfo) {
        ...
        let esApexAddress = "";
        let apexPoolAddress = "";
        let apexPoolV3Address = "";
        let apexBalance;
        let esApexBalance;
        let stakingInfo;
        let userStake;
        if (connectInfo.isArbi()) {
            apexAddress = apexRegularStakingConfig.arbi.apex;
            esApexAddress = apexRegularStakingConfig.arbi.esApex;
            apexPoolAddress = apexRegularStakingConfig.arbi.apexPoolV2;
            apexPoolV3Address = apexRegularStakingConfig.arbi.apexPoolV3;
        ...
        else if (connectInfo.isMantle()) {
            apexAddress = apexRegularStakingConfig.mantle.apex;
            esApexAddress = apexRegularStakingConfig.mantle.esApex;
            apexPoolV3Address = apexRegularStakingConfig.mantle.apexPoolV3;
            const apexErc20 = connectInfo.create(abi_1.ERC20, apexAddress);
            ...
            userStake = await this.baseApi.graphBase(apexRegularStakingConfig.mantle.graphUrlV3, (0, gql_1.getApexPoolV3StakeInfo)(), { user });
        }
        else {
            apexAddress = apexRegularStakingConfig.common.apex;
            esApexAddress = apexRegularStakingConfig.common.esApex;
            apexPoolAddress = apexRegularStakingConfig.common.apexPoolV2;
            apexPoolV3Address = apexRegularStakingConfig.common.apexPoolV3;
            const apexErc20 = connectInfo.create(abi_1.ERC20, apexAddress);
            const esApexErc20 = connectInfo.create(abi_1.ERC20, esApexAddress);
            [apexBalance, esApexBalance, stakingInfo] = await connectInfo.multiCall().call({
                balance: apexErc20.erc20Instance.balanceOf(user),
        ...
        const apexPoolStakeInfo = {
            ...
            claimRewards: apiStakeInfo.rewardList.filter((it) => successStatusSet.has(it.status)).map((it) => it.amount).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(), '0'),
            ...
            userRegularTotalApexStake: apiStakeInfo.userRegularTotalApexStake,
            userRegularTotalEsApexStake: apiStakeInfo.userRegularTotalEsApexStake,
            totalLockupAvgStakeTime: apiStakeInfo.totalLockupAvgStakeTime,
            ...
            preStake: async (token, lockPeriod) => {
                ...
                let timeFactor = "0";
                if (new bignumber_js_1.default(lockPeriod).comparedTo("0") > 0) {
                    timeFactor = new bignumber_js_1.default(lockPeriod).div(360 * 24 * 60 * 60).toFixed();
                }
                const totalFactor = ApexPoolMathTool_1.ApexPoolMathTool.totalFactor(tradingActivityFactor, timeFactor).totalFactor;
                ;
                return {
                    tradingActivityFactor,
                    t2eFactor,
            ...
            stake: async (token, amount, lockPeriod) => {
                const value = new bignumber_js_1.default(amount).multipliedBy(token === 'apex' ? (10 ** apexBalance.decimals) : (10 ** esApexBalance.decimals)).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
                if (new bignumber_js_1.default(lockPeriod).comparedTo("0") === 0) {
                    if (connectInfo.isMantle()) {
                        throw new BasicException_1.BasicException("Unsupported chains");
                    }
                    if (token === 'apex') {
                ...
                if (new bignumber_js_1.default(stakingHolding.chainId).comparedTo(connectInfo.chainId) === 0) {
                    if (!stakingHolding.isLockupStaking) {
                        ...
                        if (token === 'apex') {
                            ...
                            return apexPoolV3.unstakeEsAPEX(stakingHolding.stakeId);
                        }
                    }
                }
    ...
    async getActionsHistory(user, actions = ['stakeApex', 'unstakeApex', 'stakeEsApex', 'unstakeEsApex']) {
        ...
        const apexRegularStakingConfig = this.baseApi.address().apexRegularStaking;
        const [mainnetResult, arbiResult, mainnetV3Result, arbiV3Result, mantleV2Result] = await Promise.all([
            ...
            this.baseApi.graphBase(apexRegularStakingConfig.mantle.graphUrlV3, (0, gql_1.getApexPoolV3Logs)(), {
                user: address,
                actions: actionsSet,
                tokenName: tokensSet
            }),
        ...
        const convertApexPoolActionsHistory = (chain) => {
            return (data) => {
                ...
                let url = "";
                if (chain === 'arbitrum') {
                    url = this.baseApi.address().getScanTxUrl(this.baseApi.address().arbiChainId, it.hash);
                }
                if (chain === 'ethereum') {
                    url = this.baseApi.address().getScanTxUrl(this.baseApi.address().chainId, it.hash);
                }
                if (chain === 'mantle') {
                    url = this.baseApi.address().getScanTxUrl(this.baseApi.address().mantleChainId, it.hash);
                }
                return {
                    action: action,
                    apex,
        ...
        historyRecords.push(...Array.from(mantleV2Result.stakeLogs).map(convertApexPoolActionsHistory('mantle')));
