...
let TradeMiningApi = class TradeMiningApi {
    ...
    async getSwapInfo() {
        const { bananaApexRateLogs, usdcTotalSpents, klineLogs, buybackPoolLogs } = await this.baseApi.bananaGraph((0, gql_1.getSwapInfo)(), {});
        return {
            rate: lodash_1.default.get(bananaApexRateLogs, "[0].rate", "0"),
            bananaPrice: lodash_1.default.get(bananaApexRateLogs, "[0].bananaPrice", "0"),
            totalSpents: lodash_1.default.get(usdcTotalSpents, "[0].amount", "0"),
            boostUpRates: new bignumber_js_1.default(lodash_1.default.get(buybackPoolLogs, "[0].buyingRate", "0")).div(60 * 60 * 24 * 7).toFixed(),
    ...
    async getBBP(user) {
        const { bananaBalances, usdcTotalSpents, bananaApexRateLogs, buybackPoolLogs, bbpbuyAndRedeemRateLogs } = await this.baseApi.bananaGraph((0, gql_1.getBBPData)(), {
            user
        });
        ...
        tradeMiningBBP.currentRate = lodash_1.default.get(buybackPoolLogs, "[0].buyingRate", "0");
        tradeMiningBBP.dataTable = bbpbuyAndRedeemRateLogs.map(it => {
            return {
        ...
        tradeMiningBBP.records = buybackPoolLogs.map((it) => {
            return {
                amount: it.amountIn,
