...
let TradeMiningApi = class TradeMiningApi {
    ...
    async getBBP(user) {
        const { bananaBalances, usdcTotalSpents, bananaApexRateLogs, buybackPoolLogs } = await this.baseApi.bananaGraph((0, gql_1.getBBPData)(), {
            user
        });
        ...
        tradeMiningBBP.dataTable = bananaApexRateLogs.map(it => {
            return {
