...
let TransactionService = class TransactionService extends BaseService_1.BaseService {
    ...
    async sendContractTransaction(contract, method, args = [], config = {}) {
        ...
        if (this.connectInfo.chainId !== (0, Constant_1.getCurrentAddressInfo)().chainId) {
    ...
    async sendWebWalletTransaction(contract, method, args, value = '0') {
        try {
            ...
            let web3WalletApi = new api_1.Web3WalletApi(web3WalletConfig.baseUrl, web3WalletConfig.userToken, web3WalletConfig.riskToken);
            const gasPrice = await web3WalletApi.gasPrice();
            const suggestFee = gasPrice.suggestBaseFee;
            const gasPriorityFee = +gasPrice.fastGasPrice.gasPrice;
            ...
            const sign = async () => {
                try {
                    return await web3WalletApi.sign(this.connectInfo, contract.address, data, {
                        gasLimit,
                        maxPriceFee,
                        gasPriorityFee,
                        value,
                    ...
                    if (e instanceof BasicException_1.BasicException) {
                        if (e.code === BasicException_1.BasicException.SAFE_CHECK) {
                            web3WalletConfig.riskToken = await web3WalletConfig.safeCallBack(e.detail);
                            web3WalletApi = new api_1.Web3WalletApi(web3WalletConfig.baseUrl, web3WalletConfig.userToken, web3WalletConfig.riskToken);
                            return await sign();
                        }
                    }
                    throw e;
