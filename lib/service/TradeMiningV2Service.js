...
let TradeMiningV2Service = class TradeMiningV2Service extends BaseService_1.BaseService {
    ...
    async claimApex(user, useFor, accountId, amount, expireAt, nonce, signature) {
        const apexClaimable = this.connectInfo.create(abi_1.IApexClaimable);
        amount = new bignumber_js_1.default(amount).multipliedBy(1e6).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
        return await apexClaimable.claim(user, useFor, accountId, amount, expireAt, nonce, signature);
    }
    async redeemInfo() {
        ...
        tradeMiningRedeem.bananaUsdt = new bignumber_js_1.default(tradeMiningRedeem.bananaPrice)
            .multipliedBy(tradeMiningRedeem.bananaBalance)
            .toFixed();
...
__decorate([
    ...
    __metadata("design:paramtypes", [String, String, String, String, String, String, String]),
    __metadata("design:returntype", Promise)
], TradeMiningV2Service.prototype, "claimApex", null);
