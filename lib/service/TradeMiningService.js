...
let TradeMiningService = class TradeMiningService extends BaseService_1.BaseService {
    ...
    async claim(user, useFor, amount, expireAt, nonce, signature) {
        ...
        return await bananaClaimable.claim(user, useFor, amount, expireAt, expireAt, signature);
    }
    async swapBalance() {
        const user = this.connectInfo.account;
    ...
    async swapInfo(token, amount, slippage, deadline) {
            ...
            .toFixed(0, bignumber_js_1.default.ROUND_DOWN);
        {
            ...
            const amountOut = tool_1.MathTool.getAmountOut(amountIn, reserveIn, reserveOut);
        ...
        preSwapInfo.price = new bignumber_js_1.default(new bignumber_js_1.default(bananaReserves).div(10 ** bananaBalance.decimals))
            .div(new bignumber_js_1.default(usdcReserves).div(10 ** usdcBalance.decimals))
            .toFixed();
        ...
        preSwapInfo.swap = () => {
            ...
            const transactionEventPromise = swap.instantSwapTokenToToken(inputIsBanana ? this.addressInfo.banana : this.addressInfo.usdc, inputIsBanana ? this.addressInfo.usdc : this.addressInfo.banana, amountIn, tool_1.MathTool.deadline(deadline));
