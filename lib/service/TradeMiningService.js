...
let TradeMiningService = class TradeMiningService extends BaseService_1.BaseService {
    ...
    async swapInfo(token, amount, slippage, deadline) {
        ...
        let price;
        {
            ...
            price = new bignumber_js_1.default(usdcReserves)
                ...
                .toFixed();
            //
            let amountOutValueMin = new bignumber_js_1.default(amount).div(price).multipliedBy(10 ** bananaBalance.decimals).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
            if (inputIsBanana) {
                amountOutValueMin = new bignumber_js_1.default(amount).multipliedBy(price).multipliedBy(10 ** usdcBalance.decimals).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
            }
    ...
    async liquidityInfo() {
        ...
        tradeMiningLiquidity.preAddLiquidity = async (inputToken, inputTokenAmount) => {
            ...
            const lpTokenValue = bignumber_js_1.default.min(new bignumber_js_1.default(bananaValue).multipliedBy(totalSupply).div(bananaReserves).dp(0, bignumber_js_1.default.ROUND_DOWN), new bignumber_js_1.default(usdcValue).multipliedBy(totalSupply).div(usdcReserves).dp(0, bignumber_js_1.default.ROUND_DOWN)).multipliedBy("0.999").toFixed(0, bignumber_js_1.default.ROUND_DOWN);
