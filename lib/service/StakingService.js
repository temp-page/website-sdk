...
let StakingService = class StakingService extends BaseService_1.BaseService {
    ...
    async stakingInfo() {
        ...
        stakingInfo.balance = new bignumber_js_1.default(lpBalance.balance).div(10 ** lpBalance.decimals).toFixed();
        stakingInfo.showApprove =
        ...
        stakingInfo.unlockBalance = new bignumber_js_1.default(userInfo.withdrawable).div(10 ** lpBalance.decimals).toFixed();
        ...
        stakingInfo.lockBalance = stakingInfo.lockRecords
            .map((it) => it.amount)
        ...
        stakingInfo.maxDay = tool_1.StakingMathTool.maxDay(userInfo.endTime, blockTime).day;
        stakingInfo.userEarnedBANA = new bignumber_js_1.default(userInfo.rewardClaimable).div(1e18).toFixed();
        // 用户LP价值
        ...
        stakingInfo.preStake = async (amount, day) => {
            ...
            const userNewShares = new bignumber_js_1.default(amountValue)
                .plus(new bignumber_js_1.default(amountValue).multipliedBy(3).multipliedBy(period).div(ONE_YEAR))
                .plus(userInfo.getUserShares)
                .toFixed(0, bignumber_js_1.default.ROUND_DOWN);
            const totalShares = userInfo.totalShares;
            const newUserTVL = tool_1.StakingMathTool.tvl(new bignumber_js_1.default(stakingInfo.unlockBalance)
                .plus(stakingInfo.lockBalance)
                .plus(amount)
                .multipliedBy(1e18)
                .toFixed(), new bignumber_js_1.default(lpBalance.totalSupply).toFixed(), new bignumber_js_1.default(usdcReserves).toFixed()).tvl;
        ...
        stakingInfo.stake = async (amount, day) => {
            ...
            if (new bignumber_js_1.default(period).comparedTo('0') > 0) {
                ...
                const expireAtArrays = getStakingInfoResult.stakingDepositedLogs
                    .filter((it) => new bignumber_js_1.default(it.expireAt).comparedTo(expireAt) < 0 &&
                    new bignumber_js_1.default(it.expireAt).comparedTo(blockTime) > 0)
