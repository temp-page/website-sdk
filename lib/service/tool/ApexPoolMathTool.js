...
class ApexPoolMathTool {
     ...
     * 计算 totalFactor
     */
    static totalFactor(t2eFactor, timeFactor) {
        const totalFactor = new bignumber_js_1.default('1').plus(t2eFactor).plus(timeFactor).toFixed();
        return {
            totalFactor,
        };
    }
    static avgStakeTime(apexStakeInfo, esApexStakeInfo) {
        const weightTimeStakeAmount = new bignumber_js_1.default(apexStakeInfo.lastUserStakeAmount).multipliedBy(apexStakeInfo.avgStakeTime)
            .plus(new bignumber_js_1.default(esApexStakeInfo.lastUserStakeAmount).multipliedBy(esApexStakeInfo.avgStakeTime))
            .toFixed();
        const totalStakeAmount = new bignumber_js_1.default(apexStakeInfo.lastUserStakeAmount).plus(esApexStakeInfo.lastUserStakeAmount).toFixed();
